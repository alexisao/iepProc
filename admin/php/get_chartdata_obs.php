...
for ($i=1; $i <=5 ; $i++) { 
	...
	$q1="SELECT * FROM tbl_observaciones WHERE ob_estado=1 AND ob_fecha BETWEEN '".$dias[$i].$tinicio."' AND '".$dias[$i].$tfinal."';";
	$r1=mysql_query($q1,$con->connect());
	$c1=mysql_num_rows($r1);
	/*Cuenta para las solicitudes de Comunicaciones se_tipo_sol=2*/
	$q2="SELECT * FROM tbl_observaciones WHERE ob_estado=2 AND ob_fecha BETWEEN '".$dias[$i].$tinicio."' AND '".$dias[$i].$tfinal."';";
